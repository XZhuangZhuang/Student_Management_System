...
void Interface::DisplayByClass()
{
	...
	if (choose == 1) {
		...
		vector<undergraduate> student;//将满足条件的学生存储到该向量中
		while (p != stu1.end())
		{
		...
		Display(3, count, student.begin(), student.end());//打印
		return;
	}
	else if (choose == 2) {
		vector<graduate>::iterator p = stu2.begin();
		vector<graduate> student;//将满足条件的研究生添加到该向量中
		while (p != stu2.end())
		{
		...
		Display(4, count, student.begin(), student.end());//打印
		return;
	}
	else
	{
...
void Interface::DisplayByMajor()
{
	...
	{
		vector<undergraduate>::iterator p = stu1.begin();
		vector<undergraduate> student;//将满足条件的本科生存储到该向量中
		while (p != stu1.end())
		{
		...
		vector<graduate>::iterator p = stu2.begin();
		vector<graduate> student;//将满足条件的研究生存储到该向量中
		while (p != stu2.end())
		{
			if (p->getMajorNumber() == id)
			{
